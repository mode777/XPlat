...
namespace XPlat.NanoVg
{
	public static class GlNanoVG
	{
		                               ...
		                               ref NVGscissor scissor, float width, float fringe, float strokeThr)
		{
			...
			{
				NanoVgApi.nvgTransformInverse(invxform, scissor.xform);
			...
			if (paint.image != 0)
			{
				...
				if ((tex.flags & (int)NVGimageFlags.NVG_IMAGE_FLIPY) != 0)
				{
					...
					NanoVgApi.nvgTransformScale(flipped, 1.0f, -1.0f);
					NanoVgApi.nvgTransformMultiply(flipped, paint.xform);
					NanoVgApi.nvgTransformInverse(invxform, flipped);
				}
				else
				{
					NanoVgApi.nvgTransformInverse(invxform, paint.xform);
				}
		...
		static void glnvg__setUniforms(GLNVGcontext gl, int uniformOffset, int image)
		{
			...
			*/
			...
			unsafe { fixed(float* p = farr) { GL.Uniform4fv(lt, NanoVgApi.NANOVG_GL_UNIFORMARRAY_SIZE, p); } }//frag.uniformArray); 
		...
		public static void nvgCreateGL(ref NVGcontext ctx, int flags)
		{
			...
			NanoVgApi.nvgCreateInternal(ref params_, out ctx);
		}
	}
