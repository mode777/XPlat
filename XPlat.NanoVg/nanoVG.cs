...
namespace XPlat.NanoVg
{
    ...
    public static class NanoVgApi
    {
        ...
        {
            NVGstate state = nvg__getState(ctx);
            state.fontId = FontStashApi.fonsGetFontByName(ctx.fs, font);
        }
        ...
        public static void nvgCreateInternal(ref NVGparams params_, out NVGcontext ctx)
        {
            ...
            ctx.fs = FontStashApi.fonsCreateInternal(ref fontParams);
        ...
        static void nvgDeleteInternal(NVGcontext ctx)
        {
            ...
            if (ctx.fs != null)
                FontStashApi.fonsDeleteInternal(ctx.fs);
        ...
        public static void nvgQuadTo(NVGcontext ctx, float cx, float cy, float x, float y)
        {
            var x0 = ctx.commandx;
            var y0 = ctx.commandy;
            var vals = new float[]{ (float)NVGcommands.NVG_BEZIERTO,
                                                ...
                                                NVGglyphPosition[] positions, int maxPositions)
        {
            ...
            if (state.fontId == FontStashApi.FONS_INVALID)
                return 0;
            ...
            FontStashApi.fonsSetSize(ref ctx.fs, state.fontSize * scale);
            FontStashApi.fonsSetSpacing(ref ctx.fs, state.letterSpacing * scale);
            FontStashApi.fonsSetBlur(ref ctx.fs, state.fontBlur * scale);
            FontStashApi.fonsSetAlign(ctx.fs, (FONSalign)state.textAlign);
            FontStashApi.fonsSetFont(ref ctx.fs, state.fontId);

            FontStashApi.fonsTextIterInit(ctx.fs, ref iter, x * scale, y * scale, string_);
            prevIter = iter;
            while (FontStashApi.fonsTextIterNext(ctx.fs, ref iter, ref q) != 0)
            {
                if (iter.prevGlyphIndex < 0 && nvg__allocTextAtlas(ctx) > 0)
                { // can not retrieve glyph?
                    iter = prevIter;
                    FontStashApi.fonsTextIterNext(ctx.fs, ref iter, ref q); // try again
                }
                prevIter = iter;
        ...
        public static void nvgTextBoxBounds(NVGcontext ctx, float x, float y, float breakRowWidth, string string_, float[] bounds)
        {
            ...
            NanoVgApi.nvgTextMetrics(ctx, ref fnull, ref fnull, ref lineh);
            ...
            FontStashApi.fonsSetAlign(ctx.fs, (FONSalign)state.textAlign);
            FontStashApi.fonsSetFont(ref ctx.fs, state.fontId);
            FontStashApi.fonsLineBounds(ctx.fs, 0, ref rminy, ref rmaxy);
            rminy *= invscale;
            rmaxy *= invscale;
        ...
        public static float nvgTextBounds(NVGcontext ctx, float x, float y, string string_, float[] bounds)
        {
            ...
            width = FontStashApi.fonsTextBounds(ref ctx.fs, x * scale, y * scale, string_, bounds);
            if (bounds != null)
            {
                // Use line bounds for height.
                FontStashApi.fonsLineBounds(ctx.fs, y * scale, ref bounds[1], ref bounds[3]);
                bounds[0] *= invscale;
                bounds[1] *= invscale;
                bounds[2] *= invscale;
                bounds[3] *= invscale;
        ...
        public static void nvgTextMetrics(NVGcontext ctx, ref float ascender, ref float descender, ref float lineh)
        {
            ...
            FontStashApi.fonsVertMetrics(ref ctx.fs, ref ascender, ref descender, ref lineh);
            ascender *= invscale;
            descender *= invscale;
            lineh *= invscale;
        }
                                            ...
                                            float breakRowWidth, NVGtextRow[] rows, int maxRows)
        {
            ...
            FontStashApi.fonsTextIterInit(ctx.fs, ref iter, 0, 0, string_);
            prevIter = iter;
            while (FontStashApi.fonsTextIterNext(ctx.fs, ref iter, ref q) != 0)
            {
                // can not retrieve glyph?
        ...
        static void nvg__flushTextTexture(NVGcontext ctx)
        {
            ...
            if (FontStashApi.fonsValidateTexture(ctx.fs, dirty) != 0)
            {
                int fontImage = ctx.fontImages[ctx.fontImageIdx];
                // Update texture
                if (fontImage != 0)
                {
                    int iw = 0, ih = 0;
                    byte[] data = FontStashApi.fonsGetTextureData(ctx.fs, ref iw, ref ih);
                    int x = dirty[0];
                    int y = dirty[1];
        ...
        public static int nvgAddFallbackFontId(NVGcontext ctx, int baseFont, int fallbackFont)
        {
            if (baseFont == -1 || fallbackFont == -1)
                return 0;
            return FontStashApi.fonsAddFallbackFont(ctx.fs, baseFont, fallbackFont);
        }
    ...
    public struct NVGcolor
    {
        ...
        {
            byte strToByte(int start, int len) => byte.Parse(hexString.Substring(start, len), NumberStyles.AllowHexSpecifier);
            ...
            if(hexString.Length > 6){
                ...
                b = strToByte(3, 1);
                b = (byte)(b | b << 4);
                a = hexString.Length > 4
                    ? strToByte(4, 1)
                    : (byte)15;
                a = (byte)(a | a << 4);
            }

            return NanoVgApi.nvgRGBA(r, g, b, a);
        }
