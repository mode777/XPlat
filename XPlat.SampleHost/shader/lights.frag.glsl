...
const vec3 uAmbientColor = vec3(0.19, 0.16, 0.19);
...
{    
    ...
    gl_FragColor = vec4(result + (tex.xyz * uAmbientColor), 1.0);
}
...
vec3 CalcPointLight(vec4 tex, PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // combine results
    vec3 frag = vec3(tex) * light.color * light.intensity * 5.0 * attenuation;
    //vec3 frag = vec3(texture2D(uTexture, vUv)) * vec3(1.0,1.0,1.0) * 10.0 * attenuation;

    //vec3 ambient = light.ambient * frag;
    vec3 diffuse = diff * frag;
    vec3 specular = spec * frag;
