    ...
    public class BoxLayout : Layout
    {
        ...
        public override void PerformLayout (NVGcontext ctx, Widget widget)
        {
            ...
            for (int i = 0; childCount > i; ++i)
            {
                ...
                switch (this.alignment)
                {
                    case Alignment.Min:
                    {
                        pos.Component(axis2, pos.Component(axis2) + this.margin);
                    }
                    break;
                    ...
                    case Alignment.Max:
                    {
                        pos.Component(axis2, pos.Component(axis2) + containerSize.Component(axis2) - targetSize.Component(axis2) - this.margin * 2); // why margin * 2 (instead of margin * 1)?
                    }
                    break;
                    case Alignment.Fill:
                    {
                        pos.Component(axis2, pos.Component(axis2) + this.margin);
                        targetSize.Component(axis2, (0 < fs.Component(axis2)) ? fs.Component(axis2) : (containerSize.Component(axis2) - this.margin * 2));
                    }
                    break;
                    default:
                    break;
                ...
                position += ((int)targetSize.Component(axis1) + this.spacing);
            }
        }
        ...
        public override Vector2 GetPreferredSize (NanoVGDotNet.NVGcontext ctx, Widget widget)
        {
            ...
            for (int i = 0; childCount > i; ++i)
            {
                ...
                size.Component(axis1, size.Component(axis1) + targetSize.Component(axis1) + this.spacing);
                size.Component(axis2, Math.Max(size.Component(axis2), targetSize.Component(axis2) + 2 * this.margin));
            }
            ret = size;
            ret.Y += offsetY;
