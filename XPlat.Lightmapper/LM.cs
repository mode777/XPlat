...
//         public delegate float lm_weight_func(float cos_theta, object userdata);

//         const uint LM_NONE = 0;
//         const uint LM_UNSIGNED_BYTE = GL.UNSIGNED_BYTE;
//         const uint LM_UNSIGNED_SHORT = GL.UNSIGNED_SHORT;
//         const uint LM_UNSIGNED_INT = GL.UNSIGNED_INT;
//         const uint LM_FLOAT = GL.FLOAT;
...
//         //     int positionsType, void *positionsXYZ, int positionsStride,                              // triangle mesh in object space.
//         //     int normalsType, void *normalsXYZ, int normalsStride,                                    // optional normals for the mesh in object space (Use LM_NONE type in case you only need flat surfaces).
//         //     int lightmapCoordsType, void *lightmapCoordsUV, int lightmapCoordsStride,                // lightmap atlas texture coordinates for the mesh [0..1]x[0..1] (integer types are normalized to 0..1 range).
//         //     int count, int indicesType LM_DEFAULT_VALUE(LM_NONE), void *indices LM_DEFAULT_VALUE(0));// if mesh indices are used, count = number of indices else count = number of vertices.
...
//         // bool lmBegin(lm_context ctx,
//         //     int* outViewport4,                                                                                 // output of the current viewport: { x, y, w, h }. use these to call GL.Viewport()!
...
//         const int LM_ALL_CHANNELS = 0x0f;

//         // float lmImageMin(float *image, int w, int h, int c, int m LM_DEFAULT_VALUE(LM_ALL_CHANNELS));                    // find the minimum value (across the specified channels)
//         // float lmImageMax(float *image, int w, int h, int c, int m LM_DEFAULT_VALUE(LM_ALL_CHANNELS));                    // find the maximum value (across the specified channels)
...
//         // void lmImageDilate(float *image, float *outImage, int w, int h, int c);                                          // widen the populated non-zero areas by 1 pixel.
//         // void lmImageSmooth(float *image, float *outImage, int w, int h, int c);                                          // simple box filter on only the non-zero values.
//         // void lmImageDownsample(float *image, float *outImage, int w, int h, int c);                                      // downsamples [0..w]x[0..h] to [0..w/2]x[0..h/2] by avereging only the non-zero values
//         // void lmImageFtoUB(float *image, byte[] outImage, int w, int h, int c, float max LM_DEFAULT_VALUE(0.0f)); // casts a floating point image to an 8bit/channel image

//         // TGA file output helpers
//         // bool lmImageSaveTGAub(char *filename, byte[] image, int w, int h, int c);
//         // bool lmImageSaveTGAf(char *filename, float *image, int w, int h, int c, float max LM_DEFAULT_VALUE(0.0f));

//         //#endif
//         ////////////////////// END OF HEADER //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


//         static int lm_mini(int a, int b) { return a < b ? a : b; }
//         static int lm_maxi(int a, int b) { return a > b ? a : b; }
//         static int lm_absi(int a) { return a < 0 ? -a : a; }
//         static float lm_minf(float a, float b) { return a < b ? a : b; }
//         static float lm_maxf(float a, float b) { return a > b ? a : b; }
//         static float lm_absf(float a) { return a < 0.0f ? -a : a; }
//         static float lm_pmodf(float a, float b) { return (a < 0.0f ? 1.0f : 0.0f) + (a % b); } // positive mod


//         public struct lm_ivec2 { public int x, y; }
//         static lm_ivec2 lm_i2(int x, int y) { lm_ivec2 v = new lm_ivec2 { x = x, y = y }; return v; }

//         struct lm_vec2 { public float x, y; }
//         static lm_vec2 lm_v2i(int x, int y) { lm_vec2 v = new lm_vec2 { x = (float)x, y = (float)y }; return v; }
//         static lm_vec2 lm_v2(float x, float y) { lm_vec2 v = new lm_vec2 { x = x, y = y }; return v; }
//         static lm_vec2 lm_negate2(lm_vec2 a) { return lm_v2(-a.x, -a.y); }
...
//         static lm_vec2 lm_mul2(lm_vec2 a, lm_vec2 b) { return lm_v2(a.x * b.x, a.y * b.y); }
...
//         static lm_vec2 lm_pmod2(lm_vec2 a, float b) { return lm_v2(lm_pmodf(a.x, b), lm_pmodf(a.y, b)); }
//         static lm_vec2 lm_min2(lm_vec2 a, lm_vec2 b) { return lm_v2(lm_minf(a.x, b.x), lm_minf(a.y, b.y)); }
//         static lm_vec2 lm_max2(lm_vec2 a, lm_vec2 b) { return lm_v2(lm_maxf(a.x, b.x), lm_maxf(a.y, b.y)); }
//         static lm_vec2 lm_abs2(lm_vec2 a) { return lm_v2(lm_absf(a.x), lm_absf(a.y)); }
//         static lm_vec2 lm_floor2(lm_vec2 a) { return lm_v2(MathF.Floor(a.x), MathF.Floor(a.y)); }
//         static lm_vec2 lm_ceil2(lm_vec2 a) { return lm_v2(MathF.Ceiling(a.x), MathF.Ceiling(a.y)); }
...
//         static float lm_length2sq(lm_vec2 a) { return a.x * a.x + a.y * a.y; }
//         static float lm_length2(lm_vec2 a) { return MathF.Sqrt(lm_length2sq(a)); }
//         static lm_vec2 lm_normalize2(lm_vec2 a) { return lm_div2(a, lm_length2(a)); }
//         static bool lm_finite2(lm_vec2 a) { return float.IsFinite(a.x) && float.IsFinite(a.y); }

//         struct lm_vec3 { public float x, y, z; }
//         static lm_vec3 lm_v3(float x, float y, float z) { lm_vec3 v = new lm_vec3 { x = x, y = y, z = z }; return v; }
//         static lm_vec3 lm_negate3(lm_vec3 a) { return lm_v3(-a.x, -a.y, -a.z); }
//         static lm_vec3 lm_add3(lm_vec3 a, lm_vec3 b) { return lm_v3(a.x + b.x, a.y + b.y, a.z + b.z); }
//         static lm_vec3 lm_sub3(lm_vec3 a, lm_vec3 b) { return lm_v3(a.x - b.x, a.y - b.y, a.z - b.z); }
//         static lm_vec3 lm_mul3(lm_vec3 a, lm_vec3 b) { return lm_v3(a.x * b.x, a.y * b.y, a.z * b.z); }
//         static lm_vec3 lm_scale3(lm_vec3 a, float b) { return lm_v3(a.x * b, a.y * b, a.z * b); }
//         static lm_vec3 lm_div3(lm_vec3 a, float b) { return lm_scale3(a, 1.0f / b); }
//         static lm_vec3 lm_pmod3(lm_vec3 a, float b) { return lm_v3(lm_pmodf(a.x, b), lm_pmodf(a.y, b), lm_pmodf(a.z, b)); }
//         static lm_vec3 lm_min3(lm_vec3 a, lm_vec3 b) { return lm_v3(lm_minf(a.x, b.x), lm_minf(a.y, b.y), lm_minf(a.z, b.z)); }
//         static lm_vec3 lm_max3(lm_vec3 a, lm_vec3 b) { return lm_v3(lm_maxf(a.x, b.x), lm_maxf(a.y, b.y), lm_maxf(a.z, b.z)); }
//         static lm_vec3 lm_abs3(lm_vec3 a) { return lm_v3(lm_absf(a.x), lm_absf(a.y), lm_absf(a.z)); }
//         static lm_vec3 lm_floor3(lm_vec3 a) { return lm_v3(MathF.Floor(a.x), MathF.Floor(a.y), MathF.Floor(a.z)); }
//         static lm_vec3 lm_ceil3(lm_vec3 a) { return lm_v3(MathF.Ceiling(a.x), MathF.Ceiling(a.y), MathF.Ceiling(a.z)); }
//         static float lm_dot3(lm_vec3 a, lm_vec3 b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
//         static lm_vec3 lm_cross3(lm_vec3 a, lm_vec3 b) { return lm_v3(a.y * b.z - b.y * a.z, a.z * b.x - b.z * a.x, a.x * b.y - b.x * a.y); }
//         static float lm_length3sq(lm_vec3 a) { return a.x * a.x + a.y * a.y + a.z * a.z; }
//         static float lm_length3(lm_vec3 a) { return MathF.Sqrt(lm_length3sq(a)); }
//         static lm_vec3 lm_normalize3(lm_vec3 a) { return lm_div3(a, lm_length3(a)); }
//         static bool lm_finite3(lm_vec3 a) { return float.IsFinite(a.x) && float.IsFinite(a.y) && float.IsFinite(a.z); }
...
//         static int lm_leftOf(lm_vec2 a, lm_vec2 b, lm_vec2 c)
//         {
//             float x = lm_cross2(lm_sub2(b, a), lm_sub2(c, b));
//             return x < 0 ? -1 : (x > 0 ? 1 : 0);
//         }

//         static bool lm_lineIntersection(lm_vec2 x0, lm_vec2 x1, lm_vec2 y0, lm_vec2 y1, ref lm_vec2 res)
//         {
...
//         static int lm_convexClip(lm_vec2[] poly, int nPoly, lm_vec2[] clip, int nClip, lm_vec2[] res)
//         {
//             int nRes = nPoly;
//             int dir = lm_leftOf(clip[0], clip[1], clip[2]);
//             for (int i = 0, j = nClip - 1; i < (nClip > 0 && nRes > 0 ? 1 : 0); j = i++)
//             {
//                 if (i != 0)
...
//                 {
//                     lm_vec2 v1 = poly[k], x = default(lm_vec2);
//                     int side1 = lm_leftOf(clip[j], clip[i], v1);
//                     if (side0 + side1 == 0 && side0 > 0 && lm_lineIntersection(clip[j], clip[i], v0, v1, ref x))
//                         res[nRes++] = x;
//                     if (k == nPoly - 1)
//                         break;
//                     if (side1 != -dir)
...
//         class lm_context_mesh
//         {
//             public float[] modelMatrix;
//             public float[] normalMatrix = new float[9];
//             public byte[] positions;
//             public int positionsType;
//             public int positionsStride;
//             public byte[] normals;
//             public int normalsType;
//             public int normalsStride;
//             public byte[] uvs;
//             public int uvsType;
//             public int uvsStride;
//             public byte[] indices;
...
//             public lm_vec3[] p = new lm_vec3[3];
//             public lm_vec3[] n = new lm_vec3[3];
//             public lm_vec2[] uv = new lm_vec2[3];
//         }

//         struct lm_rasterizer
//         {
...
//         struct lm_sample
//         {
//             public lm_vec3 position;
//             public lm_vec3 direction;
//             public lm_vec3 up;
//         }

//         struct lm_hemisphere_side
//         {
//             public int side;
//         }

//         class lm_context_mesh_pos
//         {
//             public int pass;
//             public int passCount;
//             public lm_triangle triangle = new lm_triangle();
//             public lm_rasterizer rasterizer;
//             public lm_sample sample;
//             public lm_hemisphere_side hemisphere;
//         }
...
//         struct lm_hemisphere_pass
//         {
//             public uint programID;
//             public uint hemispheresTextureID;
//             public uint weightsTextureID;
//             public uint weightsTexture;
//         }

//         struct lm_downsample_pass
//         {
//             public uint programID;
//             public uint hemispheresTextureID;
//         }

//         struct lm_hemisphere_storage
//         {
//             public uint texture;
//             public lm_ivec2 writePosition;
//             public lm_ivec2[] toLightmapLocation;
//         }
...
//             public float cameraToSurfaceDistanceModifier;
//             public lm_colorf clearColor;
//             public int fbHemiCountX;
//             public int fbHemiCountY;
//             public int fbHemiIndex;
//             public lm_ivec2[] fbHemiToLightmapLocation;
//             public uint[] fbTexture = new uint[2];
//             public uint[] fb = new uint[2];
//             public uint fbDepth;
//             public uint vao;
//             public lm_hemisphere_pass firstPass;
//             public lm_downsample_pass downsamplePass;
//             public lm_hemisphere_storage storage;
//         }

//         class lm_context
//         {
//             public lm_context_mesh mesh = new lm_context_mesh();
//             public lm_context_mesh_pos meshPosition = new lm_context_mesh_pos();
//             public lm_lightmap lightmap;
//             public lm_hemisphere hemisphere = new lm_hemisphere();
...
//         static int lm_passStepSize(lm_context ctx)
//         {
//             int shift = ctx.meshPosition.passCount / 3 - (ctx.meshPosition.pass - 1) / 3;
//             int step = 1 << shift;
//             return step;
//         }

//         static int lm_passOffsetX(lm_context ctx)
//         {
//             if (!(ctx.meshPosition.pass > 0))
//                 return 0;
//             int passType = (ctx.meshPosition.pass - 1) % 3;
//             int halfStep = lm_passStepSize(ctx) >> 1;
//             return passType != 1 ? halfStep : 0;
//         }

//         static int lm_passOffsetY(lm_context ctx)
//         {
//             if (!(ctx.meshPosition.pass > 0))
//                 return 0;
//             int passType = (ctx.meshPosition.pass - 1) % 3;
...
//             return ctx.lightmap.data.AsSpan((y * ctx.lightmap.width + x) * ctx.lightmap.channels, ctx.lightmap.channels).ToArray();
//         }

//         static void lm_setLightmapPixel(lm_context ctx, int x, int y, float[] inp)
//         {
//             //assert(x >= 0 && x < ctx.lightmap.width && y >= 0 && y < ctx.lightmap.height);
//             int offset = (y * ctx.lightmap.width + x) * ctx.lightmap.channels;
//             for (int j = 0; j < ctx.lightmap.channels; j++)
//                 ctx.lightmap.data[offset + j] = inp[j];
//         }

//         const float lm_baseAngle = 0.1f;
//         static readonly float[][] lm_baseAngles = {
//      new[] { lm_baseAngle, lm_baseAngle + 1.0f / 3.0f, lm_baseAngle + 2.0f / 3.0f },
//      new[] { lm_baseAngle + 1.0f / 3.0f, lm_baseAngle + 2.0f / 3.0f, lm_baseAngle },
//      new[] { lm_baseAngle + 2.0f / 3.0f, lm_baseAngle, lm_baseAngle + 1.0f / 3.0f }
//  };
...
//                                 zero = false;
//                             if (MathF.Abs(neighbors[i][j] - avg[j]) > ctx.interpolationThreshold)
//                                 interpolate = false;
//                         }
//                         if (zero)
//                             interpolate = false;
...
//             float cameraToSurfaceDistance = (1.0f + ctx.hemisphere.cameraToSurfaceDistanceModifier) * ctx.hemisphere.zNear * MathF.Sqrt(2.0f);
...
//             const float lm_pi = 3.14159265358979f;
//             float phi = 2.0f * lm_pi * lm_baseAngles[ry][rx] + 0.1f * (float)Random.Shared.NextDouble();
//             ctx.meshPosition.sample.up = lm_normalize3(lm_add3(lm_scale3(side, MathF.Cos(phi)), lm_scale3(up, MathF.Sin(phi))));
//             return true;
//         }
...
//             GL.BindVertexArray(ctx.hemisphere.vao);
...
//             GL.BindFramebuffer(GL.FRAMEBUFFER, ctx.hemisphere.fb[fbWrite]);
//             GL.FramebufferTexture2D(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, GL.TEXTURE_2D, ctx.hemisphere.fbTexture[fbWrite], 0);
//             GL.Viewport(0, 0, (uint)(outHemiSize * ctx.hemisphere.fbHemiCountX), (uint)(outHemiSize * ctx.hemisphere.fbHemiCountY));
//             GL.UseProgram(ctx.hemisphere.firstPass.programID);
//             GL.Uniform1i((int)ctx.hemisphere.firstPass.hemispheresTextureID, 0);
//             GL.ActiveTexture(GL.TEXTURE0);
//             GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.fbTexture[fbRead]);
//             GL.Uniform1i((int)ctx.hemisphere.firstPass.weightsTextureID, 1);
//             GL.ActiveTexture(GL.TEXTURE1);
//             GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.firstPass.weightsTexture);
//             GL.ActiveTexture(GL.TEXTURE0);
...
//             //GL.BindFramebuffer( GL.READ_FRAMEBUFFER, ctx.hemisphere.fb[fbWrite]);
//             //GL.ReadBuffer( GL.COLOR_ATTACHMENT0);
//             //float *image = new float[3 * w * h];
//             //GL.ReadPixels(0, 0, w, h,  GL.RGB,  GL.FLOAT, image);
//             //lmImageSaveTGAf("firstpass.png", image, w, h, 3);
//             //delete[] image;


//             // downsampling passes
//             GL.UseProgram(ctx.hemisphere.downsamplePass.programID);
//             GL.Uniform1i((int)ctx.hemisphere.downsamplePass.hemispheresTextureID, 0);
//             while (outHemiSize > 1)
//             {
//                 var tmp = fbRead;
//                 fbRead = fbWrite;
...
//             GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.storage.texture);
//             GL.CopyTexSubImage2D(GL.TEXTURE_2D, 0,
//                 ctx.hemisphere.storage.writePosition.x, ctx.hemisphere.storage.writePosition.y,
//                 0, 0, (uint)(ctx.hemisphere.fbHemiCountX), (uint)(ctx.hemisphere.fbHemiCountY));
...
//             float[] hemi = new float[ctx.lightmap.width * ctx.lightmap.height * 4];
//             GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.storage.texture);
//             GL.GetTexImage(GL.TEXTURE_2D, 0, GL.RGBA, GL.FLOAT, hemi);
...
//                     if (lmUV.x >= 0)
//                     {
//                         float[] c = hemi.AsSpan((y * ctx.lightmap.width + x) * 4, 4).ToArray();
//                         float validity = c[3];
//                         float[] lm = ctx.lightmap.data.AsSpan((lmUV.y * ctx.lightmap.width + lmUV.x) * ctx.lightmap.channels, ctx.lightmap.channels).ToArray();
//                         if (!(lm[0] < 0) && validity > 0.9)
//                         {
//                             float scale = 1.0f / validity;
//                             switch (ctx.lightmap.channels)
//                             {
//                                 case 1:
//                                     lm[0] = lm_maxf((c[0] + c[1] + c[2]) * scale / 3.0f, float.MinValue);
//                                     break;
//                                 case 2:
//                                     lm[0] = lm_maxf((c[0] + c[1] + c[2]) * scale / 3.0f, float.MinValue);
...
//                                     break;
//                                 case 3:
//                                     lm[0] = lm_maxf(c[0] * scale, float.MinValue);
//                                     lm[1] = lm_maxf(c[1] * scale, float.MinValue);
//                                     lm[2] = lm_maxf(c[2] * scale, float.MinValue);
//                                     break;
//                                 case 4:
//                                     lm[0] = lm_maxf(c[0] * scale, float.MinValue);
//                                     lm[1] = lm_maxf(c[1] * scale, float.MinValue);
...
//             {
//                 // prepare hemisphere
//                 GL.BindFramebuffer(GL.FRAMEBUFFER, ctx.hemisphere.fb[0]);
//                 if (ctx.hemisphere.fbHemiIndex == 0)
//                 {
//                     // prepare hemisphere batch
//                     GL.ClearColor( // clear to valid background pixels!
...
//         static void lm_inverseTranspose(float[] m44, float[] n33)
//         {
//             if (m44 == null)
//             {
...
//             //assert(MathF.Abs(determinant) > FLT_EPSILON);
...
//         static lm_vec3 lm_transformNormal(float[] m, lm_vec3 n)
//         {
//             lm_vec3 r;
...
//         static lm_vec3 lm_transformPosition(float[] m, lm_vec3 v)
//         {
//             if (m == null)
//                 return v;
//             lm_vec3 r;
...
//         static void lm_setMeshPosition(lm_context ctx, int indicesTriangleBaseIndex)
//         {
...
//             lm_vec2 uvMin = lm_v2(float.MaxValue, float.MaxValue), uvMax = lm_v2(-float.MaxValue, -float.MaxValue);
...
//                 // decode index
//                 int vIndex = 0;
//                 switch ((uint)ctx.mesh.indicesType)
//                 {
//                     case LM_NONE:
//                         vIndex = ctx.meshPosition.triangle.baseIndex + i;
//                         break;
//                     case LM_UNSIGNED_BYTE:
//                         vIndex = ctx.mesh.indices[ctx.meshPosition.triangle.baseIndex + i];
//                         break;
//                     case LM_UNSIGNED_SHORT:
//                         vIndex = MemoryMarshal.Cast<byte, ushort>(ctx.mesh.indices.AsSpan())[ctx.meshPosition.triangle.baseIndex + i];
//                         break;
//                     case LM_UNSIGNED_INT:
//                         vIndex = (int)MemoryMarshal.Cast<byte, uint>(ctx.mesh.indices.AsSpan())[ctx.meshPosition.triangle.baseIndex + i];
//                         break;
//                     default:
//                         //assert(false);
//                         break;
...
//                 void* pPtr = ctx.mesh.positions + vIndex * ctx.mesh.positionsStride;
//                 lm_vec3 p;
...
//                     case LM_UNSIGNED_SHORT:
//                         {
//                             unsigned short* us = (unsigned short*)pPtr;
...
// void* uvPtr = ctx.mesh.uvs + vIndex * ctx.mesh.uvsStride;
// lm_vec2 uv;
...
//     case LM_UNSIGNED_SHORT:
//         {
//             unsigned short* us = (unsigned short*)uvPtr;
...
//     void* nPtr = ctx.mesh.normals + vIndices[i] * ctx.mesh.normalsStride;
//     lm_vec3 n;
...
//  static uint lm_LoadShader(GLenum type, char* source)
// {
...
// static uint lm_LoadProgram(char* vp, char* fp)
// {
...
//         return 0;
//     }
//     uint vertexShader = lm_LoadShader(GL.VERTEX_SHADER, vp);
//     uint fragmentShader = lm_LoadShader(GL.FRAGMENT_SHADER, fp);
...
//         GLint infoLen = 0;
//         GL.GetProgramiv(program, GL.INFO_LOG_LENGTH, &infoLen);
//         if (infoLen)
//         {
//             char* infoLog = (char*)malloc(sizeof(char) * infoLen);
//             GL.GetProgramInfoLog(program, infoLen, NULL, infoLog);
//             fprintf(stderr, "%s\n", infoLog);
//             free(infoLog);
//         }
...
//     GL.GenTextures(2, ctx.hemisphere.fbTexture);
//     GL.GenFramebuffers(2, ctx.hemisphere.fb);
//     GL.GenRenderbuffers(1, &ctx.hemisphere.fbDepth);

//     GL.BindRenderbuffer(GL.RENDERBUFFER, ctx.hemisphere.fbDepth);
//     GL.RenderbufferStorage(GL.RENDERBUFFER, GL.DEPTH_COMPONENT24, w[0], h[0]);
//     GL.BindFramebuffer(GL.FRAMEBUFFER, ctx.hemisphere.fb[0]);
//     GL.FramebufferRenderbuffer(GL.FRAMEBUFFER, GL.DEPTH_ATTACHMENT, GL.RENDERBUFFER, ctx.hemisphere.fbDepth);
//     for (int i = 0; i < 2; i++)
//     {
//         GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.fbTexture[i]);
//         GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_S, GL.CLAMP_TO_EDGE);
//         GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_T, GL.CLAMP_TO_EDGE);
//         GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER, GL.NEAREST);
//         GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER, GL.NEAREST);
//         GL.TexImage2D(GL.TEXTURE_2D, 0, GL.RGBA32F, w[i], h[i], 0, GL.RGBA, GL.FLOAT, 0);

//         GL.BindFramebuffer(GL.FRAMEBUFFER, ctx.hemisphere.fb[i]);
//         GL.FramebufferTexture2D(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, GL.TEXTURE_2D, ctx.hemisphere.fbTexture[i], 0);
//         GLenum status = GL.CheckFramebufferStatus(GL.FRAMEBUFFER);
//         if (status != GL.FRAMEBUFFER_COMPLETE)
//         {
//             fprintf(stderr, "Could not create framebuffer!\n");
//             GL.DeleteRenderbuffers(1, &ctx.hemisphere.fbDepth);
//             GL.DeleteFramebuffers(2, ctx.hemisphere.fb);
//             GL.DeleteTextures(2, ctx.hemisphere.fbTexture);
//             LM_FREE(ctx);
//             return NULL;
//         }
//     }
...
//     GL.GenVertexArrays(1, &ctx.hemisphere.vao);

//     // hemisphere shader (weighted downsampling of the 3x1 hemisphere layout to a 0.5x0.5 square)
//     {
//         char* vs =
//             "#version 150 core\n"
//              "vec2 ps[4] = vec2[](vec2(1, -1), vec2(1, 1), vec2(-1, -1), vec2(-1, 1));\n"
//              "void main()\n"
//              "{\n"
...
//              "layout(pixel_center_integer) in vec4 GL._FragCoord;\n" // whole integer values represent pixel centers,  GL.ARB_fragment_coord_conventions
...
//                  "vec2 in_uv = GL._FragCoord.xy * vec2(6.0, 2.0) + vec2(0.5);\n"
...
//             GL.DeleteVertexArrays(1, &ctx.hemisphere.vao);
//             GL.DeleteRenderbuffers(1, &ctx.hemisphere.fbDepth);
//             GL.DeleteFramebuffers(2, ctx.hemisphere.fb);
//             GL.DeleteTextures(2, ctx.hemisphere.fbTexture);
//             LM_FREE(ctx);
//             return NULL;
//         }
//         ctx.hemisphere.firstPass.hemispheresTextureID = GL.GetUniformLocation(ctx.hemisphere.firstPass.programID, "hemispheres");
//         ctx.hemisphere.firstPass.weightsTextureID = GL.GetUniformLocation(ctx.hemisphere.firstPass.programID, "weights");
//     }
...
//             GL.DeleteProgram(ctx.hemisphere.firstPass.programID);
//             GL.DeleteVertexArrays(1, &ctx.hemisphere.vao);
//             GL.DeleteRenderbuffers(1, &ctx.hemisphere.fbDepth);
//             GL.DeleteFramebuffers(2, ctx.hemisphere.fb);
//             GL.DeleteTextures(2, ctx.hemisphere.fbTexture);
...
//             return NULL;
//         }
//         ctx.hemisphere.downsamplePass.hemispheresTextureID = GL.GetUniformLocation(ctx.hemisphere.downsamplePass.programID, "hemispheres");
//     }

//     // hemisphere weights texture
//     GL.GenTextures(1, &ctx.hemisphere.firstPass.weightsTexture);
...
//     // delete GL. objects
//     GL.DeleteTextures(1, &ctx.hemisphere.firstPass.weightsTexture);
//     GL.DeleteTextures(1, &ctx.hemisphere.storage.texture);
//     GL.DeleteProgram(ctx.hemisphere.downsamplePass.programID);
//     GL.DeleteProgram(ctx.hemisphere.firstPass.programID);
//     GL.DeleteVertexArrays(1, &ctx.hemisphere.vao);
//     GL.DeleteRenderbuffers(1, &ctx.hemisphere.fbDepth);
//     GL.DeleteFramebuffers(2, ctx.hemisphere.fb);
...
//     double sum = 0.0;
//     for (uint y = 0; y < ctx.hemisphere.size; y++)
//     {
//         float dy = 2.0f * (y - center) / (float)ctx.hemisphere.size;
//         for (uint x = 0; x < ctx.hemisphere.size; x++)
//         {
...
//     for (uint i = 0; i < 2 * 3 * ctx.hemisphere.size * ctx.hemisphere.size; i++)
//         weights[i] *= weightScale;

//     // upload weight texture
//     GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.firstPass.weightsTexture);
//     GL.TexImage2D(GL.TEXTURE_2D, 0, GL.RG32F, 3 * ctx.hemisphere.size, ctx.hemisphere.size, 0, GL.RG, GL.FLOAT, weights);
//     LM_FREE(weights);
// }
...
//         GL.GenTextures(1, &ctx.hemisphere.storage.texture);
//     GL.BindTexture(GL.TEXTURE_2D, ctx.hemisphere.storage.texture);
//     GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_S, GL.CLAMP_TO_EDGE);
//     GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_T, GL.CLAMP_TO_EDGE);
//     GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER, GL.NEAREST);
//     GL.TexParameteri(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER, GL.NEAREST);
//     GL.TexImage2D(GL.TEXTURE_2D, 0, GL.RGBA32F, w, h, 0, GL.RGBA, GL.FLOAT, 0);
...
//         LM_FREE(ctx.lightmap.debug);
//     ctx.lightmap.debug = (byte*)LM_CALLOC(ctx.lightmap.width * ctx.lightmap.height, 3);
// #endif
// }

// void lmSetGeometry(lm_context ctx,
//     float* transformationMatrix,
//     int positionsType, void* positionsXYZ, int positionsStride,
//     int normalsType, void* normalsXYZ, int normalsStride,
//     int lightmapCoordsType, void* lightmapCoordsUV, int lightmapCoordsStride,
//     int count, int indicesType, void* indices)
// {
//     ctx.mesh.modelMatrix = transformationMatrix;
//     ctx.mesh.positions = (byte*)positionsXYZ;
//     ctx.mesh.positionsType = positionsType;
//     ctx.mesh.positionsStride = positionsStride == 0 ? sizeof(lm_vec3) : positionsStride;
//     ctx.mesh.normals = (byte*)normalsXYZ;
//     ctx.mesh.normalsType = normalsType;
//     ctx.mesh.normalsStride = normalsStride == 0 ? sizeof(lm_vec3) : normalsStride;
//     ctx.mesh.uvs = (byte*)lightmapCoordsUV;
...
// bool lmBegin(lm_context ctx, int* outViewport4, float* outView4x4, float* outProjection4x4)
// {
...
// float lmImageMin(float* image, int w, int h, int c, int m)
// {
//     assert(c > 0 && m);
...
// float lmImageMax(float* image, int w, int h, int c, int m)
// {
//     assert(c > 0 && m);
//     float maxValue = 0.0f;
//     for (int i = 0; i < w * h; i++)
...
// void lmImageDilate(float* image, float* outImage, int w, int h, int c)
// {
//     assert(c > 0 && c <= 4);
//     for (int y = 0; y < h; y++)
//     {
...
// void lmImageSmooth(float* image, float* outImage, int w, int h, int c)
// {
//     assert(c > 0 && c <= 4);
//     for (int y = 0; y < h; y++)
//     {
...
// void lmImageDownsample(float* image, float* outImage, int w, int h, int c)
// {
//     assert(c > 0 && c <= 4);
//     for (int y = 0; y < h / 2; y++)
//     {
...
// void lmImageFtoUB(float* image, byte[] outImage, int w, int h, int c, float max)
// {
//     assert(c > 0);
//     float scale = 255.0f / (max != 0.0f ? max : lmImageMax(image, w, h, c, LM_ALL_CHANNELS));
//     for (int i = 0; i < w * h * c; i++)
//         outImage[i] = (byte)lm_minf(lm_maxf(image[i] * scale, 0.0f), 255.0f);
// }

// // TGA output helpers
// static void lm_swapRandBub(byte[] image, int w, int h, int c)
// {
//     assert(c >= 3);
//     for (int i = 0; i < w * h * c; i += c)
//         LM_SWAP(byte, image[i], image[i + 2]);
// }
...
//     bool hasAlpha = c == 4;
//     byte header[18] = {
//          0, 0, (byte)(isGreyscale ? 3 : 2), 0, 0, 0, 0, 0, 0, 0, 0, 0,
//          (byte)(w & 0xff), (byte)((w >> 8) & 0xff), (byte)(h & 0xff), (byte)((h >> 8) & 0xff),
//          (byte)(8 * c), (byte)(hasAlpha ? 8 : 0)
//      };
// #if defined(_MSC_VER) && _MSC_VER >= 1400
//      FILE *file;
//      if (fopen_s(&file, filename, "wb") != 0) return false;
// #else
...
//     if (!isGreyscale)
//         lm_swapRandBub((byte*)image, w, h, c);
...
// bool lmImageSaveTGAf(char* filename, float* image, int w, int h, int c, float max)
// {
//     byte[] temp = (byte*)LM_CALLOC(w * h * c, sizeof(byte));
//     lmImageFtoUB(image, temp, w, h, c, max);
//     bool success = lmImageSaveTGAub(filename, temp, w, h, c);
//     LM_FREE(temp);
//     return success;
// }
