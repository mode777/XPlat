...
public const int LM_ALL_CHANNELS = 0x0f;
...
// bool lmImageSaveTGAub(const char *filename, const unsigned char *image, int w, int h, int c);
// bool lmImageSaveTGAf(const char *filename, const float *image, int w, int h, int c, float max LM_DEFAULT_VALUE(0.0f));
...
static float    lm_pmodf     (float   a, float   b) { return (a < 0.0f ? 1.0f : 0.0f) + (float)fmod(a, b); } // positive mod


public struct lm_ivec2 { int x, y; }
static lm_ivec2 lm_i2        (int     x, int     y) { lm_ivec2 v = { x, y }; return v; }

struct lm_vec2 { float x, y; }
...
static float    lm_length2   (lm_vec2 a           ) { return sqrtf(lm_length2sq(a)); }
static lm_vec2  lm_normalize2(lm_vec2 a           ) { return lm_div2(a, lm_length2(a)); }
static bool  lm_finite2   (lm_vec2 a           ) { return lm_finite(a.x) && lm_finite(a.y); }

struct lm_vec3 { float x, y, z; }
static lm_vec3  lm_v3        (float   x, float   y, float   z) { lm_vec3 v = { x, y, z }; return v; }
...
static lm_vec3  lm_floor3    (lm_vec3 a           ) { return lm_v3(floorf(a.x), floorf(a.y), floorf(a.z)); }
static lm_vec3  lm_ceil3     (lm_vec3 a           ) { return lm_v3(ceilf (a.x), ceilf (a.y), ceilf (a.z)); }
...
static float    lm_length3   (lm_vec3 a           ) { return sqrtf(lm_length3sq(a)); }
static lm_vec3  lm_normalize3(lm_vec3 a           ) { return lm_div3(a, lm_length3(a)); }
static bool  lm_finite3   (lm_vec3 a           ) { return lm_finite(a.x) && lm_finite(a.y) && lm_finite(a.z); }
...
bool lmImageSaveTGAf(const char *filename, const float *image, int w, int h, int c, float max)
{
